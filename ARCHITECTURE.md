...
## Design system
Warm near-black `#0C0A0B`, brand crimson `#A51E2F` (gradients `#C82A3E→#6E1220`), warm gray `#9E8D87`, gold **only** for winners.

**Ambient system:** one shared Background Engine renders the Alhasani diamond pattern (5 animated layers), cinematic lighting, vignette, floating gold dust, and pointer parallax on every app page. Styles live once in `public/css/scene.css`; behavior in `src/client/ambient.ts` (orchestrator + per-page `PRESETS`) composed from single-responsibility engines in `src/client/fx/` (`particles`, `parallax`, `animate`). Pages differ only by intensity variables (`--scene-pattern/-light/-vignette` + particle count) — ceremony brightest, leaderboard competitive-medium, profile/notifications calm, admin sober. Compositor-only animations, paused when the tab hides, disabled appropriately under `prefers-reduced-motion`/`forced-colors`. Shared UI atoms: `.hero-kicker`, `.stat-card` (components.css), `.fx-enter` stagger, `medal-pop` (tokens.css §18).

**Theming:** two-layer token system in `public/css/tokens.css` — dark (default, on `:root,[data-theme="dark"]`) and light (`[data-theme="light"]`, warm off-white `#F7F4F2` neutrals, same crimson/taupe/gold identity with text-grade darkened accents for WCAG AA). The active theme is the `data-theme` attribute on `<html>`, set by an inline head script in every page **before** the stylesheets load (no FOUC): `localStorage("ahc-theme")` → else `prefers-color-scheme` → else dark. `src/client/theme.ts` owns the toggle (in the nav on app pages, floating `.theme-fab` on login/register/ceremony), persists the choice, follows OS changes while the user hasn't chosen, and syncs open tabs via the `storage` event. Components never hardcode colors; anything color-bearing is a token overridden per theme. Glass surfaces (4.5% white + blur 20), radius 22/16/11, Plex Arabic with `tabular-nums` everywhere numbers move. Signature element: the **stadium arc** — a crimson ring cresting behind the login and every hero, echoed by the countdown as the largest type on screen. Motion: staggered page rise, countdown tick, flag wave on hover, animated distribution bars, podium staged reveal (bronze→silver→gold + fireworks), confetti reserved for achievements. `prefers-reduced-motion` respected globally.
